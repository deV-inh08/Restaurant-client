...
import { OrderStatusIcon, cn, getVietnameseOrdersStatus } from '@/lib/utils'
                ...
                <div className='flex flex-col gap-2'>
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrdersStatus(OrderStatus.Pending)}: {countObject[OrderStatus.Pending] ?? 0} đơn
                      </TooltipContent>
                    </Tooltip>
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrdersStatus(OrderStatus.Processing)}: {countObject[OrderStatus.Processing] ?? 0}{' '}
                        đơn
                      </TooltipContent>
                    </Tooltip>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {getVietnameseOrdersStatus(OrderStatus.Delivered)}: {countObject[OrderStatus.Delivered] ?? 0} đơn
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
      ...
      <div className='flex justify-start items-end gap-4 flex-wrap py-4'>
        {OrderStatusValues.map((status) => (
          <Badge variant='secondary' key={status}>
            {getVietnameseOrdersStatus(status)}: {statics.status[status] ?? 0}
          </Badge>
        ))}
      </div>
    </Fragment>
